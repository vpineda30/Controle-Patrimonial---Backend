...
import { validateForgotPassword, validateLogin, validateUser } from "../validations/user.validations";
...
export class UserService {
    public async createUser({ name, email, password }: UserDTO): Promise<void> {
        ...
        if (await repository.GetUserByEmail(email)) throw new Error("User already exists");
        await repository.CreateUser({ name, email, password });
    }

    public async loginUser(email: string, password: string): Promise<{ user: UserDTO; token: string }> {
    ...
    public async updatePassword(email: string, password: string): Promise<UserDTO> {
